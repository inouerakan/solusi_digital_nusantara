...
export default function History({data, fetchHistory, isCreateData}) {
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (isCreateData) {
                ...
                await axiosInstance.put(`/history/${data.id}`, {
                    year,
                    title,
                    description
                });
    ...
    const handleDelete = async () => {
        ...
        setIsLoading(true);
        try {
            await axiosInstance.delete(`/history/${data.id}`)
            fetchHistory();
        } catch (err) {
    ...
    return (
        <div className='flex-1 flex flex-col gap-4 p-4 border border-light-1/10 rounded-lg'>
            ...
            <form className='flex flex-col gap-4' onSubmit={handleSave}>
                ...
                <button type='submit' disabled={isLoading} className='w-full p-2 border border-light-1/10 hover:bg-dark-3'>{`${isLoading ? 'Menyimpan' : 'Simpan'}`}</button>
            </form>
            {!isCreateData && (
                <button 
                type='button' 
                ...
                onClick={handleDelete}>{`${isLoading ? 'Menghapus' : 'Hapus'}`}</button>
            )}
        </div>
    )
}